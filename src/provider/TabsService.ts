...
export class TabsService {
  ...
  public hide() {
    ...
    if (tabs !== null) {
      Object.keys(tabs).map((key) => {
        tabs[key].setAttribute("style", "transform:translateY(56px);-webkit-transform:translateY(56px);-o-transform:translateY(56px);");
      });
  ...
  public show() {
    ...
    if (tabs !== null) {
      Object.keys(tabs).map((key) => {
        tabs[key].setAttribute("style", "transform:translateY(0px);-webkit-transform:translateY(0px);-o-transform:translateY(0px);");
      });
    }
  }
}